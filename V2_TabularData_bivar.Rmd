...
#FACTOR is how categorical variables are called here
```
...
## Categorical-Categorical relationships (Crosstab)
...
Otra manera de hacer lo anterior, sería t1= table(vari,var2); t2=prop.table(t1); t3=t2*100; t4=round(t3,2)
...
...some more work:
```{r, eval=FALSE}
tablePlot3 = tablePlot2 + scale_size_continuous(range=c(0,10)) #change 10? Le decimos el mínimo tañano y que el más grande va a ser 10 veces más grande que el pequeño
...
```{r}
base  = ggplot(df.C, aes(x = reorder(Crime, Percent), y = Percent ) ) 
lolliplot1= base + geom_segment(aes(y=0, x=Crime, yend=Percent, xend=Crime), color="grey50")
...
```{r}
lolliplot3 = lolliplot2 + coord_flip()
lolliplot3
```

```{r}
df.C1 = df.C[order(df.C$Percent),]

base = ggplot(df.C1, aes(Crime,Percent))
lolliplot4=base + geom_segment(aes(y=0, x=Crime, yend=Percent, xend=Crime), colo = "grey")
lolliplot5 = lolliplot4 + geom_point()
lolliplot5 + scale_x_discrete(limits=df.C$Crime)
```
```{r}
lolliplot5 + coord_flip()
```
